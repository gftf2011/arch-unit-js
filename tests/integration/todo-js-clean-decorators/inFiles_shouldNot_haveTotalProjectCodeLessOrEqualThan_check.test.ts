...
describe('inFiles.shouldNot.haveTotalProjectCodeLessOrEqualThan scenarios (vanilla JS decorators sample)', () => {
  describe('Scenario 1: Selection SUM is > threshold (PASS)', () => {
    test("['use-cases/CreateTodo.js','use-cases/GetAllTodos.js'] should NOT be ≤ 1% of project - PASS", async () => {
  ...
  describe('Scenario 2: Selection SUM is ≤ threshold (FAIL)', () => {
    test("['main/index.js'] should NOT be ≤ 90% of project - FAIL", async () => {
      for (const includeMatcher of includeMatchers) {
        for (const { webpack } of webpacks) {
          const options: Options = {
            extensionTypes: ['**/*.js'],
          ...
          try {
            ...
            expect(errorMessage).toContain(
              `Violation - Rule: project files in files '[**/main/index.js]' should not have total project code less or equal than: 0.9\n\n`,
            );
    ...
    test("['infra/InMemoryTodoRepository.js'] should NOT be ≤ 75% of project - FAIL", async () => {
      for (const includeMatcher of includeMatchers) {
        for (const { webpack } of webpacks) {
          const options: Options = {
            extensionTypes: ['**/*.js'],
          ...
          try {
            ...
            const errorMessage = (error as Error).message;
            expect(errorMessage).toContain(
              `Violation - Rule: project files in files '[**/infra/InMemoryTodoRepository.js]' should not have total project code less or equal than: 0.75\n\n`,
            );
      ...
      for (const includeMatcher of includeMatchers) {
        for (const { webpack } of webpacks) {
          ...
          try {
            ...
            const errorMessage = (error as Error).message;
            expect(errorMessage).toContain(
              `Violation - Rule: project files in files '[]' should not have total project code less or equal than: 0.5\n\n`,
            );
      ...
      for (const includeMatcher of includeMatchers) {
        for (const { webpack } of webpacks) {
          ...
          try {
            ...
            const errorMessage = (error as Error).message;
            expect(errorMessage).toContain(
              `Violation - Rule: project files in files '[**/use-cases/CreateTodo.js]' should not have total project code less or equal than: 0\n\n`,
            );
          }
        }
      }
      ...
      for (const includeMatcher of includeMatchers) {
        for (const { webpack } of webpacks) {
          ...
          try {
            ...
            const errorMessage = (error as Error).message;
            expect(errorMessage).toContain(
              `Violation - Rule: project files in files '[**/use-cases/CreateTodo.js]' should not have total project code less or equal than: 1.2\n\n`,
            );
          }
        }
      }
