...
describe('inFiles.shouldNot.haveLocGreaterOrEqualThan scenarios (vanilla JS decorators sample)', () => {
  describe('Scenario 1: All selected files have LOC LESS than the threshold (PASS)', () => {
    test("['use-cases/CreateTodo.js','use-cases/GetAllTodos.js'] should NOT have LOC >= 16 - PASS", async () => {
  ...
  describe('Scenario 2: ANY selected file has LOC ≥ threshold (FAIL)', () => {
    test("['main/index.js'] should NOT have LOC >= 33 - FAIL (exact match)", async () => {
      for (const includeMatcher of includeMatchers) {
        for (const { webpack } of webpacks) {
          const options: Options = {
            extensionTypes: ['**/*.js'],
          ...
          try {
            ...
            expect(errorMessage).toContain(
              `Violation - Rule: project files in files '[**/main/index.js]' should not have L.O.C. greater or equal than: 33\n\n`,
            );
      ...
      for (const includeMatcher of includeMatchers) {
        for (const { webpack } of webpacks) {
          ...
          try {
            ...
            const errorMessage = (error as Error).message;
            expect(errorMessage).toContain(
              `Violation - Rule: project files in files '[**/infra/InMemoryTodoRepository.js, **/domain/Todo.js]' should not have L.O.C. greater or equal than: 10\n\n`,
            );
      ...
      for (const includeMatcher of includeMatchers) {
        for (const { webpack } of webpacks) {
          ...
          try {
            ...
            const errorMessage = (error as Error).message;
            expect(errorMessage).toContain(
              `Violation - Rule: project files in files '[]' should not have L.O.C. greater or equal than: 10\n\n`,
            );
      ...
      for (const includeMatcher of includeMatchers) {
        for (const { webpack } of webpacks) {
          ...
          try {
            ...
            const errorMessage = (error as Error).message;
            expect(errorMessage).toContain(
              `Violation - Rule: project files in files '[**/use-cases/CreateTodo.js]' should not have L.O.C. greater or equal than: 0\n\n`,
            );
      ...
      for (const includeMatcher of includeMatchers) {
        for (const { webpack } of webpacks) {
          ...
          try {
            ...
            const errorMessage = (error as Error).message;
            expect(errorMessage).toContain(
              `Violation - Rule: project files in files '[**/use-cases/CreateTodo.js]' should not have L.O.C. greater or equal than: -1\n\n`,
            );
            expect(errorMessage).toContain(`Threshold value must be greater than 0`);
          }
        }
      ...
      for (const includeMatcher of includeMatchers) {
        for (const { webpack } of webpacks) {
          ...
          try {
            ...
            const errorMessage = (error as Error).message;
            expect(errorMessage).toContain(
              `Violation - Rule: project files in files '[**]' should not have L.O.C. greater or equal than: 1`,
            );
          }
        }
      }
      ...
      for (const includeMatcher of includeMatchers) {
        for (const { webpack } of webpacks) {
          ...
          try {
            ...
            const errorMessage = (error as Error).message;
            expect(errorMessage).toContain(
              `Violation - Rule: project files in files '[**/domain/Todo.js]' should not have L.O.C. greater or equal than: 10\n\n`,
            );
            expect(errorMessage).toContain(
