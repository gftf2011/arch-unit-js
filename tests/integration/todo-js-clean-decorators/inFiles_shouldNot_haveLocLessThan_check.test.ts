...
describe('inFiles.shouldNot.haveLocLessThan scenarios (vanilla JS decorators sample)', () => {
  describe('Scenario 1: All selected files have LOC ≥ threshold (PASS)', () => {
    test("['main/index.js','use-cases/CreateTodo.js'] should NOT have LOC < 8 - PASS", async () => {
    ...
    test("['infra/InMemoryTodoRepository.js'] should NOT have LOC < 10 - PASS", async () => {
      for (const includeMatcher of includeMatchers) {
        for (const { webpack } of webpacks) {
          const options: Options = {
            extensionTypes: ['**/*.js'],
  ...
  describe('Scenario 2: ANY selected file has LOC < threshold (FAIL)', () => {
    test("['main/index.js'] should NOT have LOC < 100 - FAIL", async () => {
      for (const includeMatcher of includeMatchers) {
        for (const { webpack } of webpacks) {
          const options: Options = {
            extensionTypes: ['**/*.js'],
          ...
          try {
            ...
            expect(errorMessage).toContain(
              `Violation - Rule: project files in files '[**/main/index.js]' should not have L.O.C. less than: 100\n\n`,
            );
    ...
    test("['use-cases/CreateTodo.js','use-cases/GetAllTodos.js'] should NOT have LOC < 1000 - FAIL", async () => {
      for (const includeMatcher of includeMatchers) {
        for (const { webpack } of webpacks) {
          const options: Options = {
            extensionTypes: ['**/*.js'],
          ...
          try {
            ...
            const errorMessage = (error as Error).message;
            expect(errorMessage).toContain(
              `Violation - Rule: project files in files '[**/use-cases/CreateTodo.js, **/use-cases/GetAllTodos.js]' should not have L.O.C. less than: 1000\n\n`,
            );
    ...
    test('empty selection should FAIL', async () => {
      for (const includeMatcher of includeMatchers) {
        for (const { webpack } of webpacks) {
          ...
          const appInstance = ComponentSelectorBuilder.create(rootDir, options);
          try {
            await appInstance.projectFiles().inFiles([]).shouldNot().haveLocLessThan(10).check();

            expect(1).toBe(2);
          } catch (error) {
            const errorMessage = (error as Error).message;
            expect(errorMessage).toContain(
              `Violation - Rule: project files in files '[]' should not have L.O.C. less than: 10\n\n`,
            );
      ...
      for (const includeMatcher of includeMatchers) {
        for (const { webpack } of webpacks) {
          ...
          try {
            ...
            const errorMessage = (error as Error).message;
            expect(errorMessage).toContain(
              `Violation - Rule: project files in files '[**/use-cases/CreateTodo.js]' should not have L.O.C. less than: 0\n\n`,
            );
      ...
      for (const includeMatcher of includeMatchers) {
        for (const { webpack } of webpacks) {
          ...
          try {
            ...
            const errorMessage = (error as Error).message;
            expect(errorMessage).toContain(
              `Violation - Rule: project files in files '[**/domain/Todo.js]' should not have L.O.C. less than: 10\n\n`,
            );
            expect(errorMessage).toContain(
