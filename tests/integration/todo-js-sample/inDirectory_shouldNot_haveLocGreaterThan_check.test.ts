      ...
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directory '**/use-cases/**' should not have L.O.C. greater than: 20\n\n`,
          );
    ...
    test('"domain" should not have LOC greater than 15 - should FAIL (multiple files exceed threshold)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const errorMessage = (error as Error).message;
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directory '**/domain/**' should not have L.O.C. greater than: 15\n\n`,
          );
    ...
    test('entire project should not have LOC greater than 25 - should FAIL (some files exceed threshold)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directory '**' should not have L.O.C. greater than: 25\n\n`,
          );
          expect(errorMessage).toContain(
    ...
    test('"infra" should not have LOC greater than 30 - should FAIL (InMemoryTodoRepository.js likely exceeds threshold)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directory '**/infra/**' should not have L.O.C. greater than: 30\n\n`,
          );
          expect(errorMessage).toContain(
            `- '${rootDir}/infra/repositories/InMemoryTodoRepository.js'`,
          );
    ...
    test('"entities" should not have LOC greater than below Todo.js LOC count - should FAIL (boundary case)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directory '**/entities/**' should not have L.O.C. greater than: 21\n\n`,
          );
          expect(errorMessage).toContain(`- '${rootDir}/domain/entities/Todo.js'`);
        }
      }
    ...
    test('incorrect extension', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directory '**/entities/**' should not have L.O.C. greater than: 50\n\n`,
          );
          expect(errorMessage).toContain(
