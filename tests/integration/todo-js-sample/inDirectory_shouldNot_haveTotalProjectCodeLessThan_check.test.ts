...
describe('inDirectory.shouldNot.haveTotalProjectCodeLessThan (JS sample)', () => {
    ...
    test("'use-cases' should NOT have total project code less than 60% - FAIL", async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          expect(err.message).toContain(`- '${rootDir}/use-cases/GetAllTodos.js'`);
        }
      }
    });
  });
    ...
    test('incorrect extension', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in directory '**/use-cases/**' should not have total project code less than: 0.5`,
          );
          expect(err.message).toContain(
    ...
    test('invalid percentage: > 1 - should FAIL (invalid input)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in directory '**/infra/**' should not have total project code less than: 1.2`,
          );
          expect(err.message).toContain(
            `Percentage value must be greater than 0 and less or equal than 1`,
          );
