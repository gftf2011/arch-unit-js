...
describe('inDirectory.should.haveTotalProjectCodeLessThan (JS sample)', () => {
  describe('Scenario 1: SUM of directory bytes < allowed percentage of total project (PASS)', () => {
    test("'domain' should have total project code less than 100% - PASS", async () => {
    ...
    test("'infra' should have total project code less than 80% - PASS", async () => {
      for (const includeMatcher of includeMatchers) {
        const options: Options = {
          extensionTypes: ['**/*.js'],
          includeMatcher: [...includeMatcher],
  ...
  describe('Scenario 2: SUM of directory bytes >= allowed percentage of total project (FAIL)', () => {
    test("'main' should have total project code less than 1% - FAIL", async () => {
      for (const includeMatcher of includeMatchers) {
        const options: Options = {
          extensionTypes: ['**/*.js'],
          includeMatcher: [...includeMatcher],
        ...
        try {
          ...
          expect(err.message).toContain(
            `Violation - Rule: project files in directory '**/main/**' should have total project code less than: 0.01`,
          );
          expect(err.message).toContain(`- '${rootDir}/main/app.js'`);
        }
      }
    });

    test("'domain' should have total project code less than 0.01% - FAIL", async () => {
      for (const includeMatcher of includeMatchers) {
        const options: Options = {
          extensionTypes: ['**/*.js'],
          includeMatcher: [...includeMatcher],
        ...
        try {
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in directory '**/domain/**' should have total project code less than: 0.0001`,
          );
    ...
    test('incorrect extension', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in directory '**/use-cases/**' should have total project code less than: 0.5`,
          );
          expect(err.message).toContain(
    ...
    test('invalid percentage: 0 - should FAIL (invalid input)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in directory '**/domain/**' should have total project code less than: 0`,
          );
          expect(err.message).toContain(
    ...
    test('invalid percentage: > 1 - should FAIL (invalid input)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in directory '**/infra/**' should have total project code less than: 1.2`,
          );
          expect(err.message).toContain(
            `Percentage value must be greater than 0 and less or equal than 1`,
          );
