        ...
        test('"domain/entities" should depend on "inexistent-dependency" - should FAIL', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/entities/**' should depends on '[inexistent-dependency]'\n\n`);
        ...
        test('"use-cases" should depend on "domain" and "infra" - should FAIL (not all files have "domain")', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    const errorMessage = (error as Error).message;
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/use-cases/**' - excluding files [!**/use-cases/DeleteTodo.js, !**/use-cases/GetTodoById.js, !**/use-cases/GetAllTodos.js, !**/use-cases/UpdateTodo.js] , should depends on '[**/domain/**, **/infra/**]'\n\n`);
                    expect(errorMessage).toContain(`- '${rootDir}/use-cases/CreateTodo.js'`);
                }
            }
        });
        ...
        test('"main" should depend on "domain", "use-cases" and "inexistent" - should FAIL (missing inexistent)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    const errorMessage = (error as Error).message;
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/main/**' should depends on '[**/domain/**, **/use-cases/**, inexistent]'\n\n`);
