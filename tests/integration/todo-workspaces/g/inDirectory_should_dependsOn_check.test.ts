      ...
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directory '**/domain/entities/**' should depends on '[inexistent-dependency]'\n\n`,
          );
          expect(errorMessage).toContain(`- '${workspaceDir}/domain/entities/Todo.ts'`);
          expect(errorMessage).toContain(`- '${workspaceDir}/domain/entities/index.ts'`);
        }
      }
    });
  });
    ...
    test('"use-cases" should depend on "inexistent-dependency" - should FAIL', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          expect(errorMessage).toContain(`- '${workspaceDir}/use-cases/CreateTodo.ts'`);
          expect(errorMessage).toContain(`- '${workspaceDir}/use-cases/DeleteTodo.ts'`);
          expect(errorMessage).toContain(`- '${workspaceDir}/use-cases/GetAllTodos.ts'`);
          expect(errorMessage).toContain(`- '${workspaceDir}/use-cases/GetTodoById.ts'`);
          expect(errorMessage).toContain(`- '${workspaceDir}/use-cases/UpdateTodo.ts'`);
          expect(errorMessage).toContain(`- '${workspaceDir}/use-cases/index.ts'`);
        }
      }
    });
    ...
    test('"use-cases" should depend on "domain" and "uuid" - should FAIL (not all files have "uuid")', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directory '**/use-cases/**' - excluding [!**/index.ts] , should depends on '[**/domain/**, uuid]'\n\n`,
          );
          expect(errorMessage).toContain(`- '${workspaceDir}/use-cases/DeleteTodo.ts'`);
          expect(errorMessage).toContain(`- '${workspaceDir}/use-cases/GetAllTodos.ts'`);
          expect(errorMessage).toContain(`- '${workspaceDir}/use-cases/GetTodoById.ts'`);
    ...
    test('"main" should depend on "domain", "use-cases" and "infra" and "inexistent" - should FAIL (missing "inexistent")', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directory '**/main/**' should depends on '[**/domain/**, **/use-cases/**, **/infra/**, inexistent]'\n\n`,
          );
          expect(errorMessage).toContain(`- '${workspaceDir}/main/app.ts'`);
        }
      }
    });
  });
  ...
  describe('Edge scenarios', () => {
    ...
    test('incorrect extension', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          expect(errorMessage).toContain(
            `- '${workspaceDir}/domain/entities/Todo.ts' - mismatch in 'extensionTypes': [**/*.js]`,
          );
          expect(errorMessage).toContain(
            `- '${workspaceDir}/domain/entities/index.ts' - mismatch in 'extensionTypes': [**/*.js]`,
          );
          expect(errorMessage).toContain(
            `- '${workspaceDir}/domain/repositories/TodoRepository.ts' - mismatch in 'extensionTypes': [**/*.js]`,
          );
          expect(errorMessage).toContain(
            `- '${workspaceDir}/infra/repositories/InMemoryTodoRepository.ts' - mismatch in 'extensionTypes': [**/*.js]`,
          );
          expect(errorMessage).toContain(
            `- '${workspaceDir}/main/app.ts' - mismatch in 'extensionTypes': [**/*.js]`,
          );
          expect(errorMessage).toContain(
            `- '${workspaceDir}/use-cases/CreateTodo.ts' - mismatch in 'extensionTypes': [**/*.js]`,
          );
          expect(errorMessage).toContain(
            `- '${workspaceDir}/use-cases/DeleteTodo.ts' - mismatch in 'extensionTypes': [**/*.js]`,
          );
          expect(errorMessage).toContain(
            `- '${workspaceDir}/use-cases/GetAllTodos.ts' - mismatch in 'extensionTypes': [**/*.js]`,
          );
          expect(errorMessage).toContain(
            `- '${workspaceDir}/use-cases/GetTodoById.ts' - mismatch in 'extensionTypes': [**/*.js]`,
          );
          expect(errorMessage).toContain(
            `- '${workspaceDir}/use-cases/UpdateTodo.ts' - mismatch in 'extensionTypes': [**/*.js]`,
          );
          expect(errorMessage).toContain(
            `- '${workspaceDir}/use-cases/index.ts' - mismatch in 'extensionTypes': [**/*.js]`,
          );
        }
      }
    });
    ...
    test(`must throw error if file path is not being reached by the 'includeMatcher'`, async () => {
      const options: Options = {
        workspaceDir: '<rootDir>/packages/g',
        extensionTypes: ['**/*.ts'],
        includeMatcher: ['<rootDir>/packages/g/infra/**'],
        ignoreMatcher: ignoreMatchers,
        typescriptPath,
      };
      const appInstance = ComponentSelectorBuilder.create(rootDir, options);
      try {
        ...
        expect(errorMessage).toContain(
          `Check if dependencies in file: '${workspaceDir}/infra/repositories/InMemoryTodoRepository.ts' - are listed in package.json OR if dependency path is valid OR are reached by 'includeMatcher'`,
        );
