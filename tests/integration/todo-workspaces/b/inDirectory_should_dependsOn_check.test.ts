...
const rootDir = path.resolve(path.dirname(__filename), '..', '..', '..', 'sample', 'todo-workspaces');
...
const includeMatchers = [
  ...
  // ['./**'],
  // ['./**/'],
  // ['<rootDir>/packages/b/domain/**', '<rootDir>/packages/b/use-cases/**', '<rootDir>/packages/b/infra/**', '<rootDir>/packages/b/main/**'],
  // ['<rootDir>/packages/b/domain/**/', '<rootDir>/packages/b/use-cases/**/', '<rootDir>/packages/b/infra/**/', '<rootDir>/packages/b/main/**/'],
    ...
    test('"domain/entities" should depend on "inexistent-dependency" - should FAIL', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          expect(errorMessage).toContain(`- '${workspaceDir}/domain/entities/Todo.js'`);
        }
      }
    });
  });
    ...
    test('"use-cases" should depend on "inexistent-dependency" - should FAIL', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          expect(errorMessage).toContain(`- '${workspaceDir}/use-cases/DeleteTodo.js'`);
          expect(errorMessage).toContain(`- '${workspaceDir}/use-cases/GetAllTodos.js'`);
          expect(errorMessage).toContain(`- '${workspaceDir}/use-cases/GetTodoById.js'`);
          expect(errorMessage).toContain(`- '${workspaceDir}/use-cases/UpdateTodo.js'`);
        }
      }
    });
