...
describe.skip('shouldNot.haveLocLessThan scenarios', () => {
        ...
        test('"use-cases" should not have LOC less than 10 - should FAIL (GetAllTodos.ts and index.ts below threshold)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/use-cases/**' should not have L.O.C. less than: 10\n\n`);
        ...
        test('"use-cases" should not have LOC less than 20 - should FAIL (multiple files below threshold)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/use-cases/**' should not have L.O.C. less than: 20\n\n`);
        ...
        test('"entities" should not have LOC less than 30 - should FAIL (Todo.ts below threshold)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/entities/**' should not have L.O.C. less than: 30\n\n`);
        ...
        test('"domain" should not have LOC less than 40 - should FAIL (all files below threshold)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/domain/**' should not have L.O.C. less than: 40\n\n`);
                    expect(errorMessage).toContain(`- '${rootDir}/domain/entities/Todo.ts'`);
                    expect(errorMessage).toContain(`- '${rootDir}/domain/entities/index.ts'`);
        ...
        test('entire project should not have LOC less than 50 - should FAIL (all files below threshold)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**' should not have L.O.C. less than: 50\n\n`);
                    expect(errorMessage).toContain(`- '${rootDir}/domain/entities/Todo.ts'`);
                    expect(errorMessage).toContain(`- '${rootDir}/domain/entities/index.ts'`);
                    expect(errorMessage).toContain(`- '${rootDir}/domain/repositories/TodoRepository.ts'`);
        ...
        test('"main" should not have LOC less than 41 - should FAIL (app.ts below threshold)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/main/**' should not have L.O.C. less than: 41\n\n`);
                    expect(errorMessage).toContain(`- '${rootDir}/main/app.ts'`);
                }
            }
        });
        ...
        test('projectFiles.inDirectory("**/nonexistent/**").shouldNot().haveLocLessThan(10).check() - should throw error (no files exist)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/nonexistent/**' should not have L.O.C. less than: 10\n\n`);
        ...
        test('threshold of 0 should always throw error (invalid threshold)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/entities/**' should not have L.O.C. less than: 0\n\n`);
        ...
        test('threshold of -1 should always throw error (invalid threshold)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/entities/**' should not have L.O.C. less than: -1\n\n`);
                    expect(errorMessage).toContain(`Threshold value must be greater than 0`);
                }
            }
        });
        ...
        test('incorrect extension', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/entities/**' should not have L.O.C. less than: 10\n\n`);
