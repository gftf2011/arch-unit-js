      ...
      for (const [includeMatcher] of includeMatchers) {
        ...
        try {
          ...
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directory '**/entities/**' should not have name '*Todo.ts'\n\n`,
          );
    ...
    test('"infra/repositories" should not have name "*Repository.ts" - should FAIL (matches)', async () => {
      for (const [includeMatcher] of includeMatchers) {
        ...
        try {
          ...
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directory '**/infra/repositories/**' should not have name '*Repository.ts'\n\n`,
          );
          expect(errorMessage).toContain(
    ...
    test('"infra/repositories" should not have name "InMemory*Repository.ts" - should FAIL (matches)', async () => {
      for (const [includeMatcher] of includeMatchers) {
        ...
        try {
          ...
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directory '**/infra/repositories/**' should not have name 'InMemory*Repository.ts'\n\n`,
          );
          expect(errorMessage).toContain(
            `- '${rootDir}/infra/repositories/InMemoryTodoRepository.ts'`,
          );
    ...
    test('"use-cases" should not have name "*.ts" - should FAIL (all match wildcard)', async () => {
      for (const [includeMatcher] of includeMatchers) {
        ...
        try {
          ...
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directory '**/use-cases/**' should not have name '*.ts'\n\n`,
          );
