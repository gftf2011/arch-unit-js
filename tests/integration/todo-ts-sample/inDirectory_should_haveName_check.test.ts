            ...
            for (const [includeMatcher] of includeMatchers) {
                ...
                const appInstance = ComponentSelectorBuilder.create(rootDir, options);
                 try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/use-cases/**' should have name '*UseCase.ts'\n\n`);
        ...
        test('"domain/entities" should have name "*Repository.ts" - should FAIL (none match)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/entities/**' should have name '*Repository.ts'\n\n`);
        ...
        test('"infra/repositories" should have name "*Entity.ts" - should FAIL (none match)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/infra/repositories/**' should have name '*Entity.ts'\n\n`);
        ...
        test('"use-cases" should have name "*Todo.ts" - should FAIL (only some match)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/use-cases/**' should have name '*Todo.ts'\n\n`);
                    expect(errorMessage).toContain(`- '${rootDir}/use-cases/GetAllTodos.ts'`);
                    expect(errorMessage).toContain(`- '${rootDir}/use-cases/GetTodoById.ts'`);
                    expect(errorMessage).toContain(`- '${rootDir}/use-cases/index.ts'`);
                }
        ...
        test('incorrect extension', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/entities/**' should have name '*Todo.ts'\n\n`);
