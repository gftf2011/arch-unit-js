...
describe.skip('should.haveLocLessThan scenarios', () => {
        ...
        test('"use-cases" should have LOC less than 30 - should FAIL (UpdateTodo.ts exceeds threshold)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/use-cases/**' should have L.O.C. less than: 30\n\n`);
        ...
        test('"domain" should have LOC less than 20 - should FAIL (Todo.ts file exceeds threshold)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    const errorMessage = (error as Error).message;
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/domain/**' should have L.O.C. less than: 20\n\n`);
        ...
        test('entire project should have LOC less than 10 - should FAIL (most files exceed threshold)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**' should have L.O.C. less than: 10\n\n`);
                    expect(errorMessage).toContain(`- '${rootDir}/domain/entities/Todo.ts'`);
                    expect(errorMessage).toContain(`- '${rootDir}/domain/repositories/TodoRepository.ts'`);
        ...
        test('"main" should have LOC less than 40 - should FAIL (app.ts equals threshold)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    const errorMessage = (error as Error).message;
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/main/**' should have L.O.C. less than: 40\n\n`);
                    expect(errorMessage).toContain(`- '${rootDir}/main/app.ts'`);
                }
            }
        });
        ...
        test('"entities" should have LOC less than 28 - should FAIL (Todo.ts equals threshold)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    const errorMessage = (error as Error).message;
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/entities/**' should have L.O.C. less than: 28\n\n`);
                    expect(errorMessage).toContain(`- '${rootDir}/domain/entities/Todo.ts'`);
                }
            }
        });
        ...
        test('"infra" should have LOC less than 32 - should FAIL (InMemoryTodoRepository.ts equals threshold)', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    const errorMessage = (error as Error).message;
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/infra/**' should have L.O.C. less than: 32\n\n`);
                    expect(errorMessage).toContain(`- '${rootDir}/infra/repositories/InMemoryTodoRepository.ts'`);
                }
            }
        });
        ...
        test('incorrect extension', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/entities/**' should have L.O.C. less than: 50\n\n`);
