        ...
        test('"infra" should not only depend on "domain" and "pg" - should FAIL', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/infra/**' should not only depends on '[**/domain/**, pg]'\n\n`);
        ...
        test('"main" should not only depend on "domain", "use-cases" and "infra" and "uuid" - should FAIL', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/main/**' should not only depends on '[**/domain/**, **/use-cases/**, **/infra/**, uuid]'\n\n`);
        ...
        test('"use-cases" excluding "index.ts" should not only depend on "domain" and "uuid" - should FAIL', async () => {
            for (const [includeMatcher] of includeMatchers) {
                ...
                try {
                    ...
                    expect(errorMessage).toContain(`Violation - Rule: project files in directory '**/use-cases/**' - excluding files [!**/index.ts] , should not only depends on '[**/domain/**, uuid]'\n\n`);
