...
describe('inFile.shouldNot.haveTotalProjectCodeLessThan (NestJS clean sample)', () => {
  describe('Scenario 1: File bytes >= allowed percentage of total project (PASS)', () => {
    test("'main.ts' should NOT have total project code less than 0.01% - PASS", async () => {
    ...
    test("'modules/app.module.ts' should NOT have total project code less than 0.01% - PASS", async () => {
      for (const includeMatcher of includeMatchers) {
        const options: Options = {
          extensionTypes: ['**/*.ts'],
          includeMatcher: [...includeMatcher],
  ...
  describe('Scenario 2: File bytes < allowed percentage of total project (FAIL)', () => {
    test("'main.ts' should NOT have total project code less than 50% - FAIL", async () => {
      for (const includeMatcher of includeMatchers) {
        const options: Options = {
          extensionTypes: ['**/*.ts'],
          includeMatcher: [...includeMatcher],
        ...
        try {
          ...
          expect(err.message).toContain(
            `Violation - Rule: project files in file '**/main.ts' should not have total project code less than: 0.5\n`,
          );
    ...
    test("'modules/todo/todo.dto.ts' should NOT have total project code less than 30% - FAIL", async () => {
      for (const includeMatcher of includeMatchers) {
        const options: Options = {
          extensionTypes: ['**/*.ts'],
          includeMatcher: [...includeMatcher],
        ...
        try {
          await appInstance
            ...
            .shouldNot()
            .haveTotalProjectCodeLessThan(0.3)
            .check();
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in file '**/modules/todo/todo.dto.ts' should not have total project code less than: 0.3\n`,
          );
    ...
    test('invalid percentage: 0 - should FAIL (invalid input)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in file '**/main.ts' should not have total project code less than: 0\n`,
          );
          expect(err.message).toContain(
    ...
    test('invalid percentage: > 1 - should FAIL (invalid input)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in file '**/modules/app.module.ts' should not have total project code less than: 1.2\n`,
          );
          expect(err.message).toContain(
            `Percentage value must be greater than 0 and less or equal than 1`,
          );
