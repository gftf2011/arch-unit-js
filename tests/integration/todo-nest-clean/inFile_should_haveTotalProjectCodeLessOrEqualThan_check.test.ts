...
describe('inFile.should.haveTotalProjectCodeLessOrEqualThan (NestJS clean sample)', () => {
  describe('Scenario 1: File bytes <= allowed percentage of total project (PASS)', () => {
    ...
    test("'modules/todo/todo.dto.ts' should represent less or equal than 90% of total project bytes - PASS", async () => {
      for (const includeMatcher of includeMatchers) {
        const options: Options = {
          extensionTypes: ['**/*.ts'],
          includeMatcher: [...includeMatcher],
  ...
  describe('Scenario 2: File bytes > allowed percentage of total project (FAIL)', () => {
    test("'main.ts' should represent less or equal than 1% of total project bytes - FAIL", async () => {
      for (const includeMatcher of includeMatchers) {
        const options: Options = {
          extensionTypes: ['**/*.ts'],
          includeMatcher: [...includeMatcher],
        ...
        try {
          ...
          expect(err.message).toContain(
            `Violation - Rule: project files in file '**/main.ts' should have total project code less or equal than: 0.01\n`,
          );
    ...
    test("'modules/app.module.ts' should represent less or equal than 2% of total project bytes - FAIL", async () => {
      for (const includeMatcher of includeMatchers) {
        const options: Options = {
          extensionTypes: ['**/*.ts'],
          includeMatcher: [...includeMatcher],
        ...
        try {
          await appInstance
            ...
            .should()
            .haveTotalProjectCodeLessOrEqualThan(0.02)
            .check();
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in file '**/modules/app.module.ts' should have total project code less or equal than: 0.02\n`,
          );
    ...
    test('incorrect extension', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in file '**/main.ts' should have total project code less or equal than: 0.5\n`,
          );
          expect(err.message).toContain(
    ...
    test('invalid percentage: 0 - should FAIL (invalid input)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in file '**/main.ts' should have total project code less or equal than: 0\n`,
          );
          expect(err.message).toContain(
    ...
    test('invalid percentage: > 1 - should FAIL (invalid input)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in file '**/modules/app.module.ts' should have total project code less or equal than: 1.2\n`,
          );
          expect(err.message).toContain(
            `Percentage value must be greater than 0 and less or equal than 1`,
          );
