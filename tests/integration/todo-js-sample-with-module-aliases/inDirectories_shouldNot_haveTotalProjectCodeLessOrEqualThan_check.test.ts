...
describe('inDirectories.shouldNot.haveTotalProjectCodeLessOrEqualThan scenarios (module-alias sample)', () => {
  describe('Scenario 1: UNION SUM of directories bytes > allowed percentage (PASS)', () => {
    test('main should NOT have total project code less or equal than 1% - PASS', async () => {
  ...
  describe('Scenario 2: UNION SUM of directories bytes <= allowed percentage (FAIL)', () => {
      ...
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directories '[**/domain/**]' should not have total project code less or equal than: 0.8`,
          );
    ...
    test('use-cases should NOT have total project code less or equal than 60% - FAIL', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const errorMessage = (error as Error).message;
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directories '[**/use-cases/**]' should not have total project code less or equal than: 0.6`,
          );
    ...
    test('domain + use-cases should NOT have total project code less or equal than 90% - FAIL', async () => {
      for (const includeMatcher of includeMatchers) {
        const options: Options = {
          extensionTypes: ['**/*.js'],
          includeMatcher: [...includeMatcher],
        ...
        try {
          ...
          const errorMessage = (error as Error).message;
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directories '[**/domain/**, **/use-cases/**]' should not have total project code less or equal than: 0.9`,
          );
          expect(errorMessage).toContain(`- '${rootDir}/domain/entities/Todo.js'`);
          expect(errorMessage).toContain(`- '${rootDir}/domain/repositories/TodoRepository.js'`);
          expect(errorMessage).toContain(`- '${rootDir}/use-cases/CreateTodo.js'`);
    ...
    test('non-existent directories should throw error (no files found)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const errorMessage = (error as Error).message;
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directories '[**/nonexistent/**]' should not have total project code less or equal than: 0.5`,
          );
    ...
    test('incorrect extension', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const errorMessage = (error as Error).message;
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directories '[**]' should not have total project code less or equal than: 0.5`,
          );
          expect(errorMessage).toContain(
    ...
    test('invalid percentage: 0 - should FAIL (invalid input)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const errorMessage = (error as Error).message;
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directories '[**/domain/**, **/use-cases/**]' should not have total project code less or equal than: 0`,
          );
          expect(errorMessage).toContain(
    ...
    test('invalid percentage: > 1 - should FAIL (invalid input)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const errorMessage = (error as Error).message;
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directories '[**/infra/**, **/main/**]' should not have total project code less or equal than: 1.2`,
          );
          expect(errorMessage).toContain(
            `Percentage value must be greater than 0 and less or equal than 1`,
          );
