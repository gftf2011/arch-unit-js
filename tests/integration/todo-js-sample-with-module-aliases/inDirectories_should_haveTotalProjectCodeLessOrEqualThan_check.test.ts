...
describe('inDirectories.should.haveTotalProjectCodeLessOrEqualThan scenarios (module-alias sample)', () => {
  describe('Scenario 1: UNION SUM of directories bytes <= allowed percentage (PASS)', () => {
    test('domain + use-cases should have total project code less or equal than 100% - PASS', async () => {
    ...
    test('infra should have total project code less or equal than 90% - PASS', async () => {
      for (const includeMatcher of includeMatchers) {
        const options: Options = {
          extensionTypes: ['**/*.js'],
          includeMatcher: [...includeMatcher],
    ...
    test('main should have total project code less or equal than 75% - PASS', async () => {
      for (const includeMatcher of includeMatchers) {
        const options: Options = {
          extensionTypes: ['**/*.js'],
          includeMatcher: [...includeMatcher],
        ...
        await appInstance
          ...
          .inDirectories(['**/main/**'])
          .should()
          .haveTotalProjectCodeLessOrEqualThan(0.75)
          .check();
      }
    });
  });
  ...
  describe('Scenario 2: UNION SUM of directories bytes > allowed percentage (FAIL)', () => {
      ...
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directories '[**/main/**]' should have total project code less or equal than: 0.01`,
          );
    ...
    test('domain + use-cases should have total project code less or equal than 0.01% - FAIL', async () => {
      for (const includeMatcher of includeMatchers) {
        const options: Options = {
          extensionTypes: ['**/*.js'],
          includeMatcher: [...includeMatcher],
        ...
        try {
          ...
          const errorMessage = (error as Error).message;
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directories '[**/domain/**, **/use-cases/**]' should have total project code less or equal than: 0.0001`,
          );
    ...
    test('non-existent directories should throw error (no files found)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const errorMessage = (error as Error).message;
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directories '[**/nonexistent/**]' should have total project code less or equal than: 0.5`,
          );
    ...
    test('incorrect extension', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const errorMessage = (error as Error).message;
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directories '[**]' should have total project code less or equal than: 0.5`,
          );
          expect(errorMessage).toContain(
    ...
    test('invalid percentage: 0 - should FAIL (invalid input)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const errorMessage = (error as Error).message;
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directories '[**/domain/**, **/use-cases/**]' should have total project code less or equal than: 0`,
          );
          expect(errorMessage).toContain(
    ...
    test('invalid percentage: > 1 - should FAIL (invalid input)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const errorMessage = (error as Error).message;
          expect(errorMessage).toContain(
            `Violation - Rule: project files in directories '[**/infra/**, **/main/**]' should have total project code less or equal than: 1.2`,
          );
          expect(errorMessage).toContain(
            `Percentage value must be greater than 0 and less or equal than 1`,
          );
