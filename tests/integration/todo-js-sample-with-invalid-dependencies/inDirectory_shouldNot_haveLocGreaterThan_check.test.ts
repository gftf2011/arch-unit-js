...
describe('shouldNot.haveLocGreaterThan scenarios', () => {
  it('should throw an error if the dependency is not found', async () => {
    ...
    try {
      await appInstance.projectFiles().inDirectory('**').shouldNot().haveLocGreaterThan(1).check();
      ...
      expect(errorMessage).toContain(
        `Violation - Rule: project files in directory '**' should not have L.O.C. greater than: 1\n\n`,
      );
      expect(errorMessage).toContain(
