...
describe('should.haveLocLessOrEqualThan scenarios', () => {
  it('should throw an error if the dependency is not found', async () => {
    ...
    try {
      await appInstance.projectFiles().inDirectory('**').should().haveLocLessOrEqualThan(1).check();
      ...
      expect(errorMessage).toContain(
        `Violation - Rule: project files in directory '**' should have L.O.C. less or equal than: 1\n\n`,
      );
      expect(errorMessage).toContain(
