...
describe('inDirectory.should.haveTotalProjectCodeLessThan (JS sample with invalid deps)', () => {
  describe('Scenario 1: SUM of directory bytes < allowed percentage of total project (PASS)', () => {
    ...
    test("'use-cases' should have total project code less than 90% - PASS", async () => {
      for (const includeMatcher of includeMatchers) {
        const options: Options = {
          extensionTypes: ['**/*.js'],
          includeMatcher: [...includeMatcher],
  ...
  describe('Scenario 2: SUM of directory bytes >= allowed percentage of total project (FAIL)', () => {
    ...
    test("'use-cases' should have total project code less than 0.01% - FAIL", async () => {
      for (const includeMatcher of includeMatchers) {
        const options: Options = {
          extensionTypes: ['**/*.js'],
          includeMatcher: [...includeMatcher],
        ...
        try {
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in directory '**/use-cases/**' should have total project code less than: 0.0001`,
          );
    ...
    test('incorrect extension', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in directory '**/infra/**' should have total project code less than: 0.5`,
          );
          expect(err.message).toContain(
    ...
    test('invalid percentage: > 1 - should FAIL (invalid input)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in directory '**/use-cases/**' should have total project code less than: 1.2`,
          );
          expect(err.message).toContain(
            `Percentage value must be greater than 0 and less or equal than 1`,
          );
