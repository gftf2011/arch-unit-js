...
describe('shouldNot.dependsOn scenarios', () => {
  it('should throw an error if the dependency is not found', async () => {
    ...
    try {
      await appInstance.projectFiles().inDirectory('**').shouldNot().dependsOn('something').check();
      ...
      expect(errorMessage).toContain(
        `Violation - Rule: project files in directory '**' should not depends on '[something]'\n\n`,
      );
      expect(errorMessage).toContain(
