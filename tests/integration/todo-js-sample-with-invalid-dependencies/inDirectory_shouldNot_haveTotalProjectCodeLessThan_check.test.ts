...
describe('inDirectory.shouldNot.haveTotalProjectCodeLessThan (JS sample with invalid deps)', () => {
  describe('Scenario 1: SUM of directory bytes >= allowed percentage (PASS)', () => {
    ...
    test("'use-cases' should NOT have total project code less than 0.01% - PASS", async () => {
      for (const includeMatcher of includeMatchers) {
        const options: Options = {
          extensionTypes: ['**/*.js'],
          includeMatcher: [...includeMatcher],
      ...
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          expect(err.message).toContain(
            `Violation - Rule: project files in directory '**/domain/**' should not have total project code less than: 0.8`,
          );
    ...
    test("'use-cases' should NOT have total project code less than 60% - FAIL", async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in directory '**/use-cases/**' should not have total project code less than: 0.6`,
          );
    ...
    test('incorrect extension', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in directory '**/infra/**' should not have total project code less than: 0.5`,
          );
          expect(err.message).toContain(
    ...
    test('invalid percentage: 0 - should FAIL (invalid input)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in directory '**/domain/**' should not have total project code less than: 0`,
          );
          expect(err.message).toContain(
    ...
    test('invalid percentage: > 1 - should FAIL (invalid input)', async () => {
      for (const includeMatcher of includeMatchers) {
        ...
        try {
          ...
          const err = error as Error;
          expect(err.message).toContain(
            `Violation - Rule: project files in directory '**/use-cases/**' should not have total project code less than: 1.2`,
          );
          expect(err.message).toContain(
            `Percentage value must be greater than 0 and less or equal than 1`,
          );
