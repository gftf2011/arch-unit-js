...
export abstract class LOCAnalysisCheckable extends Checkable {
    ...
    public override async check(): Promise<void> {
        ...
        if (isThresholdValid) {
            throw new NotificationError(this.props.ruleConstruction, [new Error('Threshold value must be greater than 0')]);
        }
        await super.check();
    }
}
...
export abstract class PatternCheckable extends Checkable {
    ...
    public override async check(): Promise<void> {
        ...
        if (hasAnyEmptyChecker) {
            throw new NotificationError(this.props.ruleConstruction, [new Error('No pattern was provided for checking')]);
        }
        await super.check();
    }
}
...
export abstract class PatternCiclesCheckable extends PatternCheckable {
    ...
    public override async check(): Promise<void> {
        ...
        console.log('keys', JSON.stringify(Array.from(files.keys()), null, 2));
        console.log('values', JSON.stringify(Array.from(files.values()), null, 2));
