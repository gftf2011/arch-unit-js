...
export class SlicesSelectorBuilder {
    ...
    matching(): any {}
    forJavascript(): Omit<Omit<Omit<SlicesSelectorBuilder, 'forJavascript'>, 'forTypescript'>, 'forCss'> {
        const selector = new SlicesSelectorBuilder({ options: this.props.options });
        return selector;
    }
    forTypescript(): Omit<Omit<Omit<SlicesSelectorBuilder, 'forJavascript'>, 'forTypescript'>, 'forCss'> {
        const selector = new SlicesSelectorBuilder({ options: this.props.options });
        return selector;
    }
