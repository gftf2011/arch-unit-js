...
export class LayersSelectorBuilder {
    ...
    layer(): any {}
    forJavascript(): Omit<Omit<Omit<LayersSelectorBuilder, 'forJavascript'>, 'forTypescript'>, 'forCss'> {
        const selector = new LayersSelectorBuilder({ options: this.props.options });
        return selector;
    }
    forTypescript(): Omit<Omit<Omit<LayersSelectorBuilder, 'forJavascript'>, 'forTypescript'>, 'forCss'> {
        const selector = new LayersSelectorBuilder({ options: this.props.options });
        return selector;
    }
